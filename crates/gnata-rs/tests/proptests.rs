...
proptest! {
    ...
    fn lexer_no_panic(s in "[ -~]{0,60}") {
        let mut lexer = gnata::Lexer::new(&s);
        let mut infix = false;
        for _ in 0..200 {
            match lexer.next(infix) {
                Ok(tok) => {
                    if tok.typ == gnata::TokenType::EOF {
                        break;
                    }
                    // Toggle infix based on token type.
                    infix = matches!(
                        tok.typ,
                        gnata::TokenType::Name
                            | gnata::TokenType::Number
                            | gnata::TokenType::String
                            | gnata::TokenType::Value
                            | gnata::TokenType::Variable
                            | gnata::TokenType::RBracket
                            | gnata::TokenType::RBrace
                            | gnata::TokenType::RParen
                    );
                }
                Err(_) => break,
            }

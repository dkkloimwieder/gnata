#!/usr/bin/env bash
# Full cross-language benchmark: Go vs Rust vs WASI vs JS vs jsonata-core
...
JC_DIR="$BENCH_DIR/jsonata_core_bench"
...
JC_BIN="$JC_DIR/target/release/jsonata-core-bench"
...
echo "=== Building jsonata-core benchmark CLI (release) ==="
cd "$JC_DIR"
...
echo "payload,variant,expression,iters,go_mean_s,go_stddev_s,rust_mean_s,rust_stddev_s,wasi_mean_s,wasi_stddev_s,js_mean_s,js_stddev_s,jsonata_core_mean_s,jsonata_core_stddev_s" > "$CSV"
...
EOF
    cat > "$BENCH_DIR/_jc.sh" << EOF
#!/bin/sh
exec "$JC_BIN" -expr '$EXPR' -datafile "$DATAFILE" -n $ITERS
EOF
    chmod +x "$BENCH_DIR/_go.sh" "$BENCH_DIR/_rs.sh" "$BENCH_DIR/_wasi.sh" "$BENCH_DIR/_js.sh" "$BENCH_DIR/_jc.sh"
    ...
    hyperfine \
        ...
        -n "jsonata-core" "$BENCH_DIR/_jc.sh"

    # Extract to CSV
    python3 -c "
import json
data = json.load(open('$RESULT_FILE'))
go = rust = wasi = js = jc = {'mean': 0, 'stddev': 0}
for r in data['results']:
    if r['command'] == 'go': go = r
    elif r['command'] == 'rust': rust = r
    elif r['command'] == 'wasi': wasi = r
    elif r['command'] == 'js': js = r
    elif r['command'] == 'jsonata-core': jc = r
# Parse TAG into payload and variant
tag = '$TAG'
parts = tag.split('_', 1)
payload = parts[0]
variant = parts[1] if len(parts) > 1 else 'short'
print(f'{payload},{variant},$NAME,$ITERS,{go[\"mean\"]:.6f},{go[\"stddev\"]:.6f},{rust[\"mean\"]:.6f},{rust[\"stddev\"]:.6f},{wasi[\"mean\"]:.6f},{wasi[\"stddev\"]:.6f},{js[\"mean\"]:.6f},{js[\"stddev\"]:.6f},{jc[\"mean\"]:.6f},{jc[\"stddev\"]:.6f}')
" >> "$CSV"
...
echo ""
echo "╔══════════════════════════════════════════════════════════╗"
echo "║  Full Suite: Go vs Rust vs WASI vs JS vs jsonata-core   ║"
echo "╚══════════════════════════════════════════════════════════╝"
echo ""
...
rm -f "$BENCH_DIR/_go.sh" "$BENCH_DIR/_rs.sh" "$BENCH_DIR/_wasi.sh" "$BENCH_DIR/_js.sh" "$BENCH_DIR/_jc.sh"
